...
- `src/FlowLine.Infrastructure` — EF Core `FlowLineDbContext` (`Data/FlowLineDbContext.cs`: DbSets, relationship/delete-behavior config in `OnModelCreating`, concurrency-token bump in `SaveChanges`), the **SQLite** migrations (`Data/Migrations/`), `Data/SeedData.cs` (idempotent — no-ops if any `Workflow` row exists). References Domain. `OnModelCreating` also maps the two company-owned external tables (`ConfigureExternalTables`) — see "The two company-owned tables" below. References both the Sqlite and SqlServer EF Core providers so either can be selected at runtime.
...
### The two company-owned tables (`History`, `Staff_Table`)

These pre-exist in the company SQL Server and are **not owned by FlowLine**. Mapped in `ConfigureExternalTables` (`FlowLineDbContext.OnModelCreating`) with `ToTable(..., t => t.ExcludeFromMigrations())` so FlowLine's migrations *never* emit DDL against them — `Database.Migrate()` stays responsible only for FlowLine's own seven tables. Column names are mapped verbatim with `HasColumnName`, including the literal spaces in `"Assigne Number"` and `"Staff number"` (`Staff number` is also `ValueGeneratedNever` — it's a real key, not identity). Usage is **read-only**: `History` is an order source, `Staff_Table` a name lookup (see `OrderImportService`); nothing writes back. They only exist on the SQL Server provider — under SQLite they're absent, so `ExcludeFromMigrations` means even `EnsureCreated` skips them, which is why `OrderImportServiceTests` creates equivalent SQLite tables itself via raw `CREATE TABLE` (matching the mapped column names, spaces included) before seeding. If integration ever needs write-back (e.g. stamping `IsTested`/`Status`), that's a deliberate new decision — the current design was explicitly chosen as read-only to avoid touching production company data.
